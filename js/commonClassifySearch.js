 ...
 * 
 */
var Get_AJAXS,onclickForBuildDIV,onclickFor;
;
(function($, window, document, undefined) {
	$.fn.commonSearch = function(options, callback) {
		...
		$this.append(
			'<div class="yixuan"><table><tr class="new"><td><p class="part-title-icon-bg part-title-icon-bg-blue"><i class="glyphicon glyphicon-compressed"></i></p></td><td width="80" align="left"><strong>' +
			settings.title + '</strong></td><td><div class="yixuanNavList"></div></td></tr></table><div style="float:right;margin-top:-20px"><button class="su_click">确认</button></div></div>')
		...
		$.each(settings.dataList, function(index, value) {
			...
			if (value.multipleSelect == true) {
				var $multiBtn = $('<a id="' + value.id +
					'_multiBtn" href="javascript:void(0);" class="" data-name="moreDown"><i class="fa fa-plus"></i><span>多选</span></a>'
				);
			...
			if (selectorLength > 5 || value.more == true) {
				var $moreBtn = $('<a id="' + value.id +
					'_moreBtn" href="javascript:void(0);" class=""><span>更多</span><i class=" fa fa-chevron-down"></i></a>');
				$section.append($moreBtn);
				$moreBtn.bind("click", function() {
					moreBtnClick($moreBtn);
				});
			}
			var $shouqiBtn = $('<a id="' + value.id +
				'_shouqiBtn" href="javascript:void(0);" class="hidden" data-name="moreUp"><span>收起</span><i class=" fa fa-chevron-up"></i></a>'
			)
			...
			$.each(value.selectorList, function(idx, val) {
				...
				$p.append($a);
				var $i = $('<i id="' + val.id + 'i" data-parent_id="' + value.id + '" data-parent_title="' + value.title +
					'" data-id="' + val.id + '" data-name="' + val.name + '"></i>');
				$a.append($i);
				$i.bind("click", function() {
					$i.toggleClass("chbackground");
				});
				var $span = $('<span id="' + val.id + '_span" data-parent_id="' + value.id + '" data-parent_title="' + value.title +
					'" data-id="' + val.id + '" data-name="' + val.name + '">' + val.name + '</span>');
				$a.append($span);
				if (!$i.hasClass("ch")) {
		...
		// var $searchBtn = $('<button class="btn btn-default" type="button">添加搜索条件</button>');
	...
	onclickForBuildDIV=function onclickForBuildDIV(requestData) {
		...
		var $aa = $('<a data-parent_id="' + parentId + '">' + parentName + '：' + nameShow.substr(0, nameShow.length - 1) +
			'<span class="tit-close">×</span></a>');
		$(".yixuanNavList").append($aa);
		$aa.bind("click", function() {
			$(this).remove();
			resetData(parentId);
	...
	onclickFor=function onclickFor(_this) {
	...
	//选中所有条件后点击确认按钮查询
	$(document).on("click",".su_click",function(){
		Get_AJAXS(1,JSON.stringify($.fn.commonSearch.RequestPramData));
	})
	//搜索结果
	Get_AJAXS=function Get_AJAXS(num,datas) {
		var rUrl = $.fn.commonSearch.RequestUrl;
		var result = null;
		$.ajax({
			url: baseUrl+"search/searchsortByClassify",
			type: "post",
			xhrFields: {
